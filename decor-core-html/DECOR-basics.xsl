...
-->
<xsl:stylesheet xmlns:xforms="http://www.w3.org/2002/xforms" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:uuid="java:java.util.UUID" xmlns:local="http://art-decor.org/functions" xmlns:diff="http://art-decor.org/ns/decor/diff" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" version="2.0" exclude-result-prefixes="#all">
    ...
    <xsl:variable xmlns="" name="allDECOR" select="/decor | /decor-excerpt" as="element()?"/>
    
    <!-- cache all ids for later processing -->
    <xsl:variable xmlns="" name="allIDs" select="//ids"/>

    <!-- cache all i18n messages -->
    <xsl:variable xmlns="" name="theMESSAGES" select="doc(concat($scriptBaseUriPrefix, 'DECOR-i18n.xml'))"/>
                ...
                <xsl:choose>
                    <xsl:when test="@id">
                        <xsl:copy>
                            ...
                            <xsl:if test="conceptList/(concept|exception) and not(sourceCodeSystem)">
                                
                                <!-- check if sourceCodeSystem is used in this original value set, if not - create them -->
                                <xsl:for-each-group select="conceptList/(concept|exception)" group-by="@codeSystem">
    ...
    <xsl:variable name="projectRestUri" as="xs:string?">
        <xsl:variable name="theURI">
            <xsl:choose>
                <xsl:when test="$allDECOR/@deeplinkprefixservices[string-length() > 0]">
                    <xsl:value-of select="concat($allDECOR/@deeplinkprefixservices,'ProjectIndex')"/>
                </xsl:when>
    ...
    <xsl:variable name="projectRestUriDS" as="xs:string?">
        <xsl:variable name="theURI">
            <xsl:choose>
                <xsl:when test="$allDECOR/@deeplinkprefixservices[string-length() > 0]">
                    <xsl:value-of select="concat($allDECOR/@deeplinkprefixservices,'RetrieveTransaction')"/>
                </xsl:when>
                <xsl:otherwise>
    ...
    <xsl:variable name="projectRestUriVS" as="xs:string?">
        <xsl:variable name="theURI">
            <xsl:choose>
                <xsl:when test="$allDECOR/@deeplinkprefixservices[string-length() > 0]">
                    <xsl:value-of select="concat($allDECOR/@deeplinkprefixservices,'RetrieveValueSet')"/>
                </xsl:when>
                <xsl:otherwise>
    ...
    <xsl:variable name="projectRestUriFHIR" as="xs:string*">
        <xsl:for-each select="$projectRestURIs[@for='FHIR'][string-length(.)&gt;0]">
            <xsl:variable name="theURI">
                <xsl:value-of select="."/>
                <xsl:value-of select="$projectPrefix"/>
                <xsl:if test="$useLatestDecorVersion">
        ...
        <xsl:variable name="tempTemplates" as="element()*">
            <xsl:for-each-group select="$theV2Scenarios//representingTemplate[@ref]" group-by="concat(@ref, @flexibility[not(. = 'dynamic')])">
                ...
                <xsl:call-template name="getTemplateChain2">
                    ...
                    <xsl:with-param name="flex" select="()"/>
    ...
    <xsl:variable name="doV2ValueSets" as="element()*">
        <xsl:variable name="tempValueSets" as="element()*">
            <xsl:for-each-group group-by="concat(@valueSet | @value, @flexibility[not(. = 'dynamic')])" select="
                $allDECOR/rules/template[concat(@id, @effectiveDate) = $theV2Templates/concat(@id, @effectiveDate)]//vocabulary |
                ...
                <xsl:choose>
                    ...
                    <xsl:when test="$getMissingValueSetsFromRepo = 'true'">
                        <xsl:copy-of select="doc(concat('http://art-decor.org/decor/services/RetrieveValueSet?prefix=ad4bbr-&amp;ref=',$vsref,'&amp;effectiveDate=',if (string-length($vsflex)>0) then $vsflex else ('dynamic'),'&amp;format=xml'))//valueSet[@id][@effectiveDate]"/>
                    </xsl:when>
                </xsl:choose>
            </xsl:for-each-group>
        </xsl:variable>
    ...
    <xsl:variable name="latestVersionOrRelease" select="(//project/(version|release)[@date=max(parent::project/(version|release)/xs:dateTime(@date))])[1]" as="element()?"/>
    ...
    <xsl:template name="getTemplateChain2">
        <!-- 
        ...
        <xsl:for-each-group select="$template//element[@contains][count(ancestor-or-self::element) = count(ancestor-or-self::element[@minimumMultiplicity[. > 0] | @conformance[not(. = 'NP')] | @isMandatory[. = 'true']])] | 
            ...
            <tmp1 xmlns="">
                <xsl:choose>
                    ...
                    <xsl:when test="$theMESSAGES/*/entry[@key=$key]/text[substring(@language, 1, 2)=substring($defaultLanguage, 1, 2)]">
                        <xsl:copy-of select="$theMESSAGES/*/entry[@key=$key]/text[substring(@language, 1, 2)=substring($defaultLanguage, 1, 2)][1]/node()"/>
                    </xsl:when>
    ...
    <xsl:template name="getXFormsLabel">
        ...
        <xsl:choose>
            <xsl:when test="doc-available('DECOR.xsd')">
                <xsl:variable name="simpleTypes" select="doc('DECOR.xsd')/*/xs:simpleType | doc('DECOR-datatypes.xsd')/*/xs:simpleType" as="element()*"/>
                <xsl:choose>
                    <xsl:when test="$simpleTypes[@name=$simpleTypeKey]/*/xs:enumeration[@value=$simpleTypeValue]//xforms:label[@xml:lang=$lang]">
                        <xsl:value-of select="$simpleTypes[@name=$simpleTypeKey]/*/xs:enumeration[@value=$simpleTypeValue]//xforms:label[@xml:lang=$lang]/text()"/>
                    </xsl:when>
                    <xsl:when test="$simpleTypes[@name=$simpleTypeKey]/*/xs:enumeration[@value=$simpleTypeValue]//xforms:label[@xml:lang=$defaultLanguage]">
                        <xsl:value-of select="$simpleTypes[@name=$simpleTypeKey]/*/xs:enumeration[@value=$simpleTypeValue]//xforms:label[@xml:lang=$defaultLanguage]/text()"/>
                    </xsl:when>
                    <xsl:when test="$simpleTypes[@name=$simpleTypeKey]/*/xs:enumeration[@value=$simpleTypeValue]//xforms:label[@xml:lang='en-US']">
                        <xsl:value-of select="$simpleTypes[@name=$simpleTypeKey]/*/xs:enumeration[@value=$simpleTypeValue]//xforms:label[@xml:lang='en-US']/text()"/>
                    </xsl:when>
                    <xsl:otherwise>
    ...
    <xsl:template name="multipleReplace">
        ...
        <xsl:choose>
            <xsl:when test="$ix &gt; 0 and $ix &lt;= count($px/p)">
                ...
                -->
                <xsl:call-template name="multipleReplace">
                    <xsl:with-param name="in" select="replace($in, concat ('%%', $px/p[$ix]/@n), replace($px/p[$ix]/@v, '([^\\])\\([^\\$])', '$1\\\\$2'))"/>
                    <xsl:with-param name="px" select="$px"/>
    ...
    <xsl:template name="doDescription">
        ...
        <xsl:for-each select="$descs">
            ...
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <!-- show in grey only -->
                    <div style="color: grey;" xmlns="http://www.w3.org/1999/xhtml">
                        <xsl:choose>
                            ...
                            <xsl:when test="@language = ('de-DE', 'en-US', 'nl-NL')">
                                <xsl:call-template name="showIcon">
                                    ...
                                    <xsl:with-param name="style" select="'float: left; margin: 4px;'"/>
                                </xsl:call-template>
                                <xsl:text> </xsl:text>
                            </xsl:when>
                            <xsl:otherwise>
    ...
    <xsl:template name="showIcon">
        ...
        <img xmlns="http://www.w3.org/1999/xhtml">
            ...
            <xsl:attribute name="style" select="concat($style, ' width:16px; height:16px;')"/>
            <xsl:choose>
                ...
                <xsl:when test="$which='link11'">
                    ...
                    <!-- override size -->
                    <xsl:attribute name="style" select="concat($style, ' width:11px; height:11px;')"/>
                </xsl:when>
                ...
                <xsl:when test="$which='de-DE'">
                    ...
                    <!-- override size -->
                    <xsl:attribute name="style" select="concat($style, ' width:16px; height:11px;')"/>
                </xsl:when>
                ...
                <xsl:when test="$which='partialpublication'">
                    ...
                    <!-- override size -->
                    <xsl:attribute name="style" select="concat($style, ' width:20px; height:20px;')"/>
                </xsl:when>
    ...
    <xsl:template name="showStatusDot">
        ...
        <xsl:variable name="imgprefix" select="$theAssetsDir"/>
        <img alt="{$status}" xmlns="http://www.w3.org/1999/xhtml">
            <xsl:choose>
                ...
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        ...
                        <xsl:variable name="theCodeElement" as="element(element)*">
                            ...
                            <xsl:variable name="telms-other1" select="$rcunpacked[vocabulary[@code | @codeSystem | @valueSet] | attribute[not(@prohibited = 'true')][not(@isOptional = 'true')][@name = ('code','codeSystem')][string-length(@value) > 0]]"/>
                            <!-- get other required code element direct underneath top level -->
                            <xsl:variable name="telms-other2" select="$rcunpacked/element[@name][@minimumMultiplicity > 0][vocabulary[@code | @codeSystem | @valueSet] | attribute[not(@prohibited = 'true')][not(@isOptional = 'true')][@name = ('code','codeSystem')][string-length(@value) > 0]]"/>
                            <xsl:choose>
                                <xsl:when test="$rcunpacked[ends-with(@name, $telmname)][vocabulary[@code | @codeSystem | @valueSet] | attribute[not(@prohibited = 'true')][not(@isOptional = 'true')][@name = ('code','codeSystem')][string-length(@value) > 0]]">
                                    <!-- code -->
                                    <xsl:copy-of select="$rcunpacked[ends-with(@name, $telmname)][vocabulary[@code | @codeSystem | @valueSet] | attribute[not(@prohibited = 'true')][not(@isOptional = 'true')][@name = ('code','codeSystem')][string-length(@value) > 0]]"/>
                                </xsl:when>
                                <xsl:when test="$rcunpacked/element[@minimumMultiplicity > 0][ends-with(@name, $telmname)][vocabulary[@code | @codeSystem | @valueSet] | attribute[not(@prohibited = 'true')][not(@isOptional = 'true')][@name = ('code','codeSystem')][string-length(@value) > 0]]">
                                    <!-- element/code -->
                                    <xsl:copy-of select="$rcunpacked/element[@minimumMultiplicity > 0][ends-with(@name, $telmname)][vocabulary[@code | @codeSystem | @valueSet] | attribute[not(@prohibited = 'true')][not(@isOptional = 'true')][@name = ('code','codeSystem')][string-length(@value) > 0]]"/>
                                </xsl:when>
                                <xsl:when test="$rcunpacked/element[@minimumMultiplicity > 0]/element[ends-with(@name, $telmname)][@minimumMultiplicity > 0][vocabulary[@code | @codeSystem | @valueSet] | attribute[not(@prohibited = 'true')][not(@isOptional = 'true')][@name = ('code','codeSystem')][string-length(@value) > 0]]">
                                    <!-- element/element/code -->
                                    <xsl:copy-of select="$rcunpacked/element[@minimumMultiplicity > 0]/element[ends-with(@name, $telmname)][@minimumMultiplicity > 0][vocabulary[@code | @codeSystem | @valueSet] | attribute[not(@prohibited = 'true')][not(@isOptional = 'true')][@name = ('code','codeSystem')][string-length(@value) > 0]]"/>
                                </xsl:when>
                                        ...
                                        <xsl:when test="$parentChoice">
                                            ...
                                            <xsl:for-each select="$theCodeElement">
                                                <!-- Prohibiting attribute nullFlavor is effectively saying that the item is mandatory in a different way. Difference:
                                                    ...
                                                    element[@minimumMultiplicity = 0]/attribute[not(@name = ('nullFlavor','xsi:type'))][not(@prohibited = 'true')][not(@isOptional = 'true')]
                                                        element MAY be present, and SHALL NOT have a nullFlavor (since there is some other required attribute, other than xsi:type)
                                                -->
                                                <xsl:variable name="isMandatoryAttr" select="exists(
                                                    .[@isMandatory = 'true'] | attribute[@name = 'nullFlavor'][@prohibited = 'true'][not(@value)] | 
                                                    attribute[not(@name = ('nullFlavor','xsi:type'))][not(@prohibited = 'true')][not(@isOptional = 'true')])"/>
                                                ...
                                                <xsl:value-of select="@name"/>
                                                <xsl:choose>
                                                    <xsl:when test="string-length($whereselector) = 0 and not(contains(@name, '\['))">
                                                        <xsl:if test="$isMandatoryAttr">
    ...
    <xsl:template name="doAttributePredicate">
        ...
        <xsl:for-each select="$rcunpacked[1]/attribute[not(@prohibited = 'true')][not(@isOptional = 'true')][@name = ('typeCode','classCode','moodCode')][@value[string-length() &gt; 0]]">
            <xsl:text>[@</xsl:text>
            <xsl:value-of select="@name"/>
            <xsl:text>=</xsl:text>
            <xsl:choose>
                    ...
                    <xsl:variable name="selector" as="xs:string*">
                        <xsl:choose>
                            ...
                            <xsl:when test=".[name() = 'vocabulary'][@valueSet]">
                                ...
                                <xsl:choose>
                                    <xsl:when test="$xvs/valueSet[1]/conceptList/(include|exclude)">
    ...
    <xsl:template name="xsiTypePredicate">
        ...
        <xsl:if test="string-length($isSupportedDatatype) > 0 and $switchCreateDatatypeChecks = true()">
            ...
            <xsl:if test="$isSupportedDatatype != 'ANY'">
                <xsl:variable name="predicateString">
                    <xsl:text>(local-name-from-QName(resolve-QName(@xsi:type,.))='</xsl:text>
                    <xsl:value-of select="$dtVal"/>
                    <xsl:text>' and namespace-uri-from-QName(resolve-QName(@xsi:type,.))='</xsl:text>
                    <xsl:value-of select="$dtNS"/>
                    <xsl:text>')</xsl:text>
                    <xsl:if test="not($required)">
                        <xsl:text> or not(@xsi:type)</xsl:text>
                    </xsl:if>
                </xsl:variable>
    ...
    <xsl:function name="local:randomString2">
        ...
        <!--<xsl:value-of select="if (doc-available('http://art-decor.org/decor/services/modules/random-string.xquery')) then doc(concat('http://art-decor.org/decor/services/modules/random-string.xquery?', $gid))/random/text() else $gid"/>-->
    ...
    <xsl:function name="local:getMinimumMultiplicity" as="xs:string">
        ...
        <xsl:choose>
            <xsl:when test="$concept[@conformance='NP'] | $concept[condition/@conformance='NP'] | $concept[conditionalConcept/@conformance='NP'] | $concept[@prohibited='true'] | $concept[@isOptional='true']">
                <xsl:text>0</xsl:text>
            </xsl:when>
            ...
            <xsl:otherwise>
                <xsl:value-of select="string(min(($concept/condition[@minimumMultiplicity]/number(@minimumMultiplicity),$concept/conditionalConcept[@minimumMultiplicity]/number(@minimumMultiplicity))))"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:function>
    <xd:doc>
    ...
    <xsl:function name="local:getMaximumMultiplicity" as="xs:string">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:value-of select="string(max(($concept/condition[@maximumMultiplicity castable as xs:integer]/number(@maximumMultiplicity),$concept/conditionalConcept[@maximumMultiplicity castable as xs:integer]/number(@maximumMultiplicity))))"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:function>
    <xd:doc>
    ...
    <xsl:function name="local:getTemplateFormat" as="xs:string?">
        ...
        <xsl:if test="$template">
            <xsl:choose>
                <xsl:when test="$template/classification/@format[string-length() > 0]">
                    <xsl:value-of select="($template/classification/@format[string-length() > 0])[1]"/>
                </xsl:when>
