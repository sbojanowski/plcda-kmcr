...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
    ...
    <sch:pattern id="vsreq">
        ...
        <sch:rule context="concept[valueDomain/@type='code'][not(ancestor-or-self::*[@statusCode = ('deprecated','cancelled','rejected')])]">
            <sch:let name="locationContext" value="concat(' | Location &lt;concept ', string-join(for $att in ancestor-or-self::concept[1]/(@id, @ref, @name, name[1], @displayName, @effectiveDate, @flexibility) return concat(name($att),'=&#34;',$att,'&#34;'),' '),'&gt;')"/>
    ...
    <sch:pattern id="oneinh">
        ...
        <sch:rule context="concept[inherit][not(ancestor-or-self::*[@statusCode = ('deprecated','cancelled','rejected')])]">
            ...
            <sch:assert role="error" test="not(//concept[inherit/@ref = $ref][2])">ERROR: Inherited concept ref "<sch:value-of select="$ref"/>" is inherited twice. This project only allows single inheritance per project to avoid duplicatie IDs, since the inherited IDs are used in FHIR resources.<sch:value-of select="$locationContext"/></sch:assert>
        </sch:rule>
    </sch:pattern>
</sch:schema>