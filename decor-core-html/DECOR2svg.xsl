...
<!-- 
    ...
    Copyright (C) 2009-2015 Alexander Henket, Dr. Kai U. Heitmann
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:svg="http://www.w3.org/2000/svg" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" exclude-result-prefixes="#all" version="2.0">
    ...
    <!--<xsl:variable name="allDECOR" select="/decor" as="element(decor)?"/>-->
    ...
    <xsl:template match="transaction[@type='group']" mode="transactionGroupToSVGFunctional">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                ...
                <xsl:variable name="actorBoxes">
                    <wrap>
                        <xsl:for-each select="$actorsUnique/wrap/actor">
                            ...
                            <xsl:variable name="statusStyle" select="if ($transactionGroups//transaction[not(@statusCode=$inactiveStatusCodes)][actors/actor/@id=$id]) then 'fill-opacity: 1; stroke-opacity: 1;' else 'fill-opacity: 0.25; stroke-opacity: 0.25;'"/>
                            ...
                            <g xmlns="http://www.w3.org/2000/svg" id="actor_{@id}">
                                ...
                                <text x="{$actorBoxXoffset + ($actorBoxWidth div 2) + ((position()-1) * $actorBoxXdistance)}" y="{$actorBoxYoffset + $actorBoxMargin}" style="font-size:{$letterHeight}px;font-weight:bold;text-align:middle;line-height:100%;text-anchor:middle;fill:black;stroke:none;font-family: Verdana, Arial, sans-serif;{$statusStyle}">
                                ...
                                <xsl:choose>
                                    <xsl:when test="@type='initial'">
                                        <xsl:for-each select="actors/actor[@role='sender']">
                                            ...
                                            <xsl:for-each select="../actor[@role='receiver']">
                                                ...
                                                <rect x="{$sequenceBarX}" y="{$sequenceBarY}" height="{$sequenceBarHeight}" width="{$sequenceBarWidth}" style="fill:AliceBlue;fill-rule:evenodd;stroke:black;stroke-width:0.2;stroke-linecap:butt;stroke-linejoin:miter;stroke-miterlimit:4;stroke-dasharray:none;{$statusStyle}"/>
                                                <g id="{concat('initiatingTransaction_',generate-id(.))}">
                                                    <xsl:comment> Arrow text client to server </xsl:comment>
                                                    <xsl:text>
</xsl:text>
                                                    <text x="{$arrowLineTextX}" y="{$arrowLineTextY}" style="font-size:{$letterHeight}px;font-variant:normal;font-weight:bold;font-stretch:normal;text-align:center;line-height:125%;writing-mode:lr-tb;text-anchor:middle;fill:black;stroke:none;font-family: Verdana, Arial, sans-serif;{$statusStyle}">
                                                    ...
                                                    <path d="{concat('m ', $arrowLineX + $arrowLengthFull, ',', $arrowLineY - 5, ' 0.2428,9.99706 c 3.8451,-1.75975 7.6902,-3.51949 11.5354,-5.27924 0,-0.002 0,-0.002 0,-0.002 0,0 0,0 -0,-0.002 -3.926,-1.57108 -7.852,-3.14216 -11.7781,-4.71324 z')}" style="fill:black;fill-rule:evenodd;stroke:none;{$statusStyle}"/>
                                                </g>
                                                <xsl:if test="not(ancestor::transaction[@id=$transactionId]/following-sibling::transaction[@type='back'][actors/actor[@id=$actorId][@role='receiver']][actors/actor[@id=$actorIdBack][@role='sender']])">
                                                    ...
                                                    <rect x="{$sequenceBarReceiverXoffset}" y="{$sequenceBarY}" height="{$sequenceBarHeight}" width="{$sequenceBarWidth}" style="fill:AliceBlue;fill-rule:evenodd;stroke:black;stroke-width:0.2;stroke-linecap:butt;stroke-linejoin:miter;stroke-miterlimit:4;stroke-dasharray:none;{$statusStyle}"/>
                                                </xsl:if>
                                                <xsl:for-each select="ancestor::transaction[@id=$transactionId]/following-sibling::transaction[@type='back'][actors/actor[@id=$actorId][@role='receiver']][actors/actor[@id=$actorIdBack][@role='sender']]">
                                                    ...
                                                    <g id="{concat('respondingTransaction_',$actorId,'_',generate-id(.))}">
                                                        ...
                                                        <text x="{$arrowLineTextXBack}" y="{$arrowLineTextYBack}" style="font-size:{$letterHeight}px;font-variant:normal;font-weight:bold;font-stretch:normal;text-align:center;line-height:125%;writing-mode:lr-tb;text-anchor:middle;fill:black;stroke:none;font-family: Verdana, Arial, sans-serif;{$statusStyle}">
                                    ...
                                    <xsl:when test="@type='stationary'">
                                        <xsl:for-each select="actors/actor[@role=('sender','stationary')]">
                                            ...
                                            <g id="{concat('initiatingTransaction_',generate-id(.))}">
                                                ...
                                                <text x="{$arrowLineTextX}" y="{$arrowLineTextY}" style="font-size:{$letterHeight}px;font-variant:normal;font-weight:bold;font-stretch:normal;text-align:center;fill:black;stroke:none;font-family: Verdana, Arial, sans-serif;{$statusStyle}">
                                                    <xsl:value-of select="$transactionName"/>
                                                </text>
                                                <xsl:comment> Arrow line client to server </xsl:comment>
                                                <xsl:text>
                ...
                <xsl:variable name="sequenceLineHeight" select="if (empty($sequenceLineYmax)) then $sequenceLineYmin else ($sequenceLineYmax + ($sequenceBars/wrap/svg:g/svg:rect[@y=$sequenceLineYmax])[last()]/@height) + $sequenceLineHeightAfterLastBar - $sequenceLineYoffset"/>
                ...
                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" id="{concat('svg_',$transactionGroupId)}" version="1.1" height="{$svgHeight}" width="{$svgWidth}" style="fill:white;stroke:black;stroke-width:0;{$statusStyle}">
                    ...
                    <g id="sequenceLines">
                        <xsl:for-each select="$actorBoxes/wrap/svg:g">
                            ...
                            <path style="fill:none;stroke:black;stroke-width:1px;stroke-linecap:butt;stroke-linejoin:miter;{$statusStyle}" d="m {$sequenceLineX}, {$sequenceLineYoffset} v {$sequenceLineHeight}"/>
                        </xsl:for-each>
                    </g>
